...
function getTeamForUser(db: ReturnType<typeof getDb>, userId: number) {
  return db
    .prepare(
      `
      SELECT t.id, t.name, t.captain_user_id as captainUserId
      FROM teams t
    ...
    .get(userId) as { id: number; name: string; captainUserId: number } | undefined;
}
...
function getRegistration(db: ReturnType<typeof getDb>, teamId: number) {
  return db
    .prepare(
      `
      SELECT tt.tournament_id as tournamentId, tt.registered_at as registeredAt, t.name as tournamentName, t.status
      FROM tournament_teams tt
      LEFT JOIN tournaments t ON t.id = tt.tournament_id
      WHERE tt.team_id = ?
...
export async function GET(req: NextRequest) {
  ...
  const members = db
    ...
    .all(team.id) as { userId: number; role: string | null; status: string | null; fullName: string | null }[];
  ...
  try {
    switch (action) {
      case "create": {
        ...
        const result = db
          .prepare("INSERT INTO teams (name, captain_user_id) VALUES (?, ?)")
          .run(name, telegramId);
        const teamId = Number(result.lastInsertRowid);
        db.prepare(
          "INSERT OR IGNORE INTO team_members (team_id, user_id, role, status) VALUES (?, ?, 'captain', 'confirmed')",
      ...
      case "add_member": {
        ...
        db.prepare("INSERT OR IGNORE INTO team_members (team_id, user_id, role, status) VALUES (?, ?, 'player', 'confirmed')")
          .run(team.id, newUserId);
      ...
      case "remove_member": {
        ...
        db.prepare("DELETE FROM team_members WHERE team_id = ? AND user_id = ?").run(team.id, removeId);
        return NextResponse.json({ ok: true });
      }
      case "leave": {
        const team = getTeamForUser(db, telegramId);
        ...
        db.prepare("DELETE FROM team_members WHERE team_id = ? AND user_id = ?").run(team.id, telegramId);
        return NextResponse.json({ ok: true });
      }
      case "delete": {
        const team = getTeamForUser(db, telegramId);
