...
from db import execute, fetch, fetchrow, init_db
...
async def user_exists(user_id: int) -> bool:
    row = await fetchrow("SELECT 1 FROM users WHERE user_id = $1", user_id)
...
async def get_main_menu(user_id: int):
    ...
    row = await fetchrow("SELECT team FROM users WHERE user_id = $1", user_id)
    in_team = row and row[0]
    is_free_agent = (await fetchrow("SELECT 1 FROM free_agents WHERE user_id = $1", user_id)) is not None
...
async def start(message: Message, state: FSMContext):
    ...
    await message.answer(
        "👋 Привет, мы Vzale! Дата первого турнира 24 августа.\n\n"
...
async def choose_status(callback: CallbackQuery, state: FSMContext):
    if callback.data == "has_team":
        rows = await fetch("SELECT DISTINCT team_name FROM teams")
        if rows:
...
async def register_new_team(message: Message, state: FSMContext):
    ...
    await execute(
        "INSERT INTO users (user_id, full_name, team) VALUES ($1, $2, $3)",
        user_id,
        full_name,
        team_name,
    )
    await execute(
        "INSERT INTO teams (team_name, member_id, member_name) VALUES ($1, $2, $3)",
        team_name,
        user_id,
        full_name,
    )
    ...
    await message.answer(
        "🎉 Команда <b>{team_name}</b> успешно создана! \n\n"
...
async def join_team(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "✅ Ты добавлен в команду <b>{team_name}</b>!\n\nПодпишись чтобы ничего не пропустить:\n "
        "https://t.me/vzzale \n https://vk.com/vzale1 \n https://www.instagram.com/vzale_bb?igsh=Y2Y1Nmx5YTE4aWJp",
        reply_markup=await get_main_menu(user_id),
    )
    await state.clear()
...
async def leave_free_agents(callback: CallbackQuery):
    user_id = callback.from_user.id
    await execute("DELETE FROM free_agents WHERE user_id = $1", user_id)
    await callback.message.answer(
        "✅ Твоя анкета свободного игрока удалена.(Если хочешь добавиться в команду, то введи /start)",
        reply_markup=await get_main_menu(user_id),
    )
...
async def show_my_team(callback: CallbackQuery):
    ...
    if not row or not row[0]:
        ...
        return
    team_name = row[0]
    members = await fetch("SELECT member_name FROM teams WHERE team_name = $1", team_name)
    names = "\n".join([f"• {m[0]}" for m in members])
    await callback.message.answer(
        f"<b>🏀 Твоя команда: {team_name}</b>\n\n👥 Участники:\n{names}",
        reply_markup=await get_main_menu(user_id),
    )
...
async def show_teams(callback: CallbackQuery):
    teams = await fetch("SELECT DISTINCT team_name FROM teams")
    if not teams:
        await callback.message.answer(
            "🚫 Пока нет зарегистрированных команд.",
            reply_markup=await get_main_menu(callback.from_user.id),
    ...
    for row in teams:
        team = row[0]
        members = await fetch("SELECT member_name FROM teams WHERE team_name = $1", team)
...
async def show_free_agents(callback: CallbackQuery):
    ...
    if user_id not in ADMINS:
        ...
        return
    agents = await fetch("SELECT name, info FROM free_agents")
    if not agents:
...
async def handle_free_agent_info(message: Message, state: FSMContext):
    ...
    await execute(
        "INSERT INTO free_agents (user_id, name, info) VALUES ($1, $2, $3)",
        user_id,
        name,
        info,
    )
    await notify_admins(f"🧍 <b>Новый свободный игрок:</b>\n👤 {name}\n📋 {info}")
    await message.answer("🧍 Ты добавлен в список свободных игроков!", reply_markup=await get_main_menu(user_id))
    await state.clear()
...
async def delete_profile(callback: CallbackQuery):
    ...
    await execute("DELETE FROM teams WHERE member_id = $1", user_id)
    await execute("DELETE FROM free_agents WHERE user_id = $1", user_id)
...
async def leave_team(callback: CallbackQuery):
    ...
    if not row or not row[0]:
        ...
        return
    team = row[0]
    await execute("UPDATE users SET team = NULL WHERE user_id = $1", user_id)
    await execute("DELETE FROM teams WHERE member_id = $1", user_id)
    await callback.message.answer(
        f"🚪 Ты вышел из команды <b>{team}</b>.", reply_markup=await get_main_menu(user_id)
    )
...
async def rejoin_team(callback: CallbackQuery, state: FSMContext):
    user_id = callback.from_user.id
    row = await fetchrow("SELECT full_name FROM users WHERE user_id = $1", user_id)
    if not row:
...
async def delete_team(callback: CallbackQuery):
    team_name = callback.data.split(":")[1]
    await execute("DELETE FROM teams WHERE team_name = $1", team_name)
    await execute("UPDATE users SET team = NULL WHERE team = $1", team_name)
    await callback.message.answer(
        f"❌ Команда <b>{team_name}</b> удалена.", reply_markup=await get_main_menu(callback.from_user.id)
    )
...
async def admin_poll_results(callback: CallbackQuery):
    row = await fetchrow(
        "SELECT group_id, question, options FROM polls_group ORDER BY created_at DESC LIMIT 1"
    )
    if not row:
    ...
    poll_rows = await fetch("SELECT poll_id FROM polls WHERE group_id = $1", group_id)
    poll_ids = [r[0] for r in poll_rows]
    if not poll_ids:
        await callback.message.answer("❌ Опросов нет")
        return

    placeholders = ",".join(f"${i+1}" for i in range(len(poll_ids)))
    votes = await fetch(
    ...
    async def get_name(uid: int):
        r1 = await fetchrow("SELECT full_name FROM users WHERE user_id=$1", uid)
        if r1 and r1[0]:
            return r1[0]
        r2 = await fetchrow("SELECT name FROM free_agents WHERE user_id=$1", uid)
        if r2 and r2[0]:
            return r2[0]
        return f"id{uid}"
